########################################################################################
# This is a script will check status/stop cm agent in all the cluster nodes         #
...
def stop_all_cm_agents():
    for host in hosts:
            # Create SSH client
            ssh_client = paramiko.SSHClient()
            ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh_client.connect(hostname=host, username=ssh_username, password=ssh_password)
            stdin, stdout, stderr = ssh_client.exec_command("systemctl stop cloudera-scm-agent")
            ...
            ssh_client.close()
            print(f"stoped cloudera-scm-agent on {host}")
